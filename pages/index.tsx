        ...
        <div className="grid lg:grid-cols-3 sm:grid-cols-1 md:grid-cols-2">
          {
            PROPERTYLISTINGSAMPLE.map((item, index) => (
              ...
              <Card image={item.image} name={item.name} address={item.address} rating={item.rating} offers={item.offers} price={item.price} category={item.category} discount={item.discount} description={item.description} />
              </div>
            ))
          }
        </div>
